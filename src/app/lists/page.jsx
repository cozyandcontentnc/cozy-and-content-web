...
export default function ListsIndex() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      () => { setLists([]); setLoading(false); }
    );
    return unsub;
  }, [uid]);

  // (Optional) peek at legacy single-list count: wishlists/{uid}/items
  useEffect(() => {
    if (!uid) return;
    const qRef = query(
  ...
  return (
    <main style={{ padding: 24, fontFamily: "system-ui", background: "#faf7f2", minHeight: "100vh" }}>
      {/* Top bar */}
      ...
      <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:20 }}>
        <button onClick={onCreateList} disabled={!uid || busy} className="cc-btn">
          ➕ New List
        </button>
      ...
      {loading ? (
          ...
          {lists.map((l) => (
            ...
            >
              ...
              <div style={{ color:"#666", fontSize:13 }}>
                ...
                {l.itemCount ? `• ${l.itemCount} item${l.itemCount === 1 ? "" : "s"}` : ""}
              </div>
            </Link>
          ))}
        </div>
        ...
        <div className="cc-card" style={{ maxWidth:700 }}>
          ...
          <ul style={{ marginTop:8 }}>
            <li>Click <strong>➕ New List</strong> to create your first one, or</li>
            <li>Use <Link href="/scan">📷 Scan a Book</Link> to start adding items.</li>
          </ul>
          {legacyCount !== null && legacyCount > 0 && (
            <p style={{ marginTop:12, fontSize:13, color:"#555" }}>
              ⚠️ You have {legacyCount} item{legacyCount === 1 ? "" : "s"} in your legacy single-list
              (<code>wishlists/{uid}/items</code>). We can migrate those into a new list later.
            </p>
          )}
        </div>
      )}
